...
```{r}
Entropy <- read.csv("/Volumes/TwoTeras/1_Experiment_2/Entropy_Results/grouped_for_LMM.csv", sep =",")
...
```{r}
HumanA_Agent <- Merged_Data %>% dplyr::filter(!SubjectID %in% c(4580, 3540))
...
# Just behavioral
# No Eye-tracking
No_Eye_tracking <- update(TrialIDrandomIntercept_Agent, .~. + ContextEffectf*Agent_Action_levelf + max )
# Model summary
...
Complete_model <- update(TrialIDrandomIntercept_Agent, 
                        . ~ . + Dwelling_Time_Building_Gaze + Dwelling_Time_Agent_Gaze +   ContextEffectf*Agent_Action_levelf + mean)
# Model summary
